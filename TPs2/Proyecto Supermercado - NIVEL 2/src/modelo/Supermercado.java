...
public class Supermercado {
	...
	public Cliente traerCliente(int idCliente) {
		...
		for (int i=0; i<lstCliente.size(); i++) {
			if (lstCliente.get(i).getIdCliente() == idCliente) {
				c = new Cliente(lstCliente.get(i).getIdCliente(), lstCliente.get(i).getCliente(), lstCliente.get(i).getDni(), lstCliente.get(i).getDireccion());
			}
		}
		return c;
	}
	...
	public Carrito traerCarrito(LocalTime hora) {
		Carrito c = null;
	...
	public Carrito traerCarrito(Producto producto) {
		Carrito c = null;
		for (int i = 0; i < lstCarrito.size(); i++) {
			for (int j = 0; j < lstCarrito.get(i).getLstItemCarrito().size(); j++) {
				if (lstCarrito.get(i).getLstItemCarrito().get(j).getProducto().equals(producto)) {
					c = lstCarrito.get(i);
				}
			}
		}
	...
	public float calcularTotal(Cliente cliente) throws Exception {
		...
		for (int i = 0; i < lstCarrito.size(); i++) {
			if (lstCarrito.get(i).getCliente().equals(cliente)) {
				encontrado = true;
				for (int j = 0; j < lstCarrito.get(i).getLstItemCarrito().size(); j++) {
					total = total + lstCarrito.get(i).getLstItemCarrito().get(j).calcularSubTotal();
				}
			}
		}
		if (!encontrado) {
